...
def company_card_text(c: dict) -> str:
    lines = [f"<b>🏢 {c['short_name']}</b>\n"]
    lines.append(f"<b>Полное наименование:</b> {c['full_name']}")
    lines.append(f"<b>ИНН:</b> {c['inn']}  <b>КПП:</b> {c['kpp']}")
    if c.get("reg_number"):
        lines.append(f"<b>Рег. номер:</b> {c['reg_number']}")
    lines.append(f"<b>Адрес:</b> {c['address']}")
    if c.get("kio") and c["kio"] != "-":
        lines.append(f"<b>КИО:</b> {c['kio']}")
    if c.get("inn_rf") and c["inn_rf"] != "-":
        lines.append(f"<b>ИНН/КПП в РФ:</b> {c['inn_rf']} / {c.get('kpp_rf','')}")
    lines.append(f"\n<b>Банк:</b> {c.get('bank_name','—')}")
    lines.append(f"<b>Р/счёт:</b> {c.get('bank_account','—')}")
    lines.append(f"<b>БИК:</b> {c.get('bank_bik','—')}")
    if c.get("wallet"):
        lines.append(f"\n<b>Кошелёк:</b> <code>{c['wallet']}</code>")
    return "\n".join(lines)
...
async def start_edit(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=edit_company_fields_kb(cid)
    )
    await cb.answer()
